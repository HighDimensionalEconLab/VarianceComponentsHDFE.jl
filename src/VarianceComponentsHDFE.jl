...
function real_main()
    ...
    if parsed_args["write_detailed_CSV"]
        ...
        output = DataFrame(observation = obs === nothing ? missings(max_length) : replace(obs, nothing => missing),
                           first_id = first_id_output === nothing ? missings(max_length) : replace(first_id_output, nothing => missing),
                           second_id = second_id_output === nothing ? missings(max_length) :  replace(second_id_output, nothing => missing),
                           y = y_output === nothing ? missings(max_length) :  replace(y_output, nothing => missing),
                        #    beta = vcat(β,missings(max(max_length-length(β),0))),
                           D_alpha = Dalpha === nothing ? missings(max_length) :  replace(Dalpha, nothing => missing),
                           F_psi = Fpsi === nothing ? missings(max_length) :  replace(Fpsi, nothing => missing),
                           Pii = Pii === nothing ? missings(max_length) :  replace(Pii, nothing => missing),
                           Bii_first = Bii_first === nothing ? missings(max_length) :  replace(Bii_first, nothing => missing),
                           Bii_second = Bii_second === nothing ? missings(max_length) :  replace(Bii_second, nothing => missing),
                           Bii_cov = Bii_cov === nothing ? missings(max_length) :  replace(Bii_cov, nothing => missing),
